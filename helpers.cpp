...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring){
    ...
    vector<float> windowRow3(3);
  	windowRow3.push_back(corner_prob);
  	windowRow3.push_back(adjacent_prob);
  	windowRow3.push_back(corner_prob);*/
  	window.push_back({corner_prob, adjacent_prob, corner_prob});
  	...
  	for(int row=0; row<grid.size(); row++){
        for(int col=0; col<grid[0].size(); col++){
            ...
            for(int dx=-1; dx<grid.size()-1; dx++){
            	for(int dy=-1; dx<grid[0].size()-1; dy++){
                	float mult = window[dx+1][dy+1];
                    if((row+dy) < 0){
                    	new_i = (row + dy) + grid.size();
                    }else{
                    	new_i = (row + dy) % grid.size();
                    }
                    if((col+dx) < 0){
                    	new_j = (col + dx) + grid[0].size();
                    }else{
                    	new_j = (col + dx) % grid[0].size();// % grid[0].size();	 
                    }
                    //if(new_i == -1){new_i = 0;}
                  	//if(new_j == -1){new_j = 0;}
                    //newRow.push_back(newGrid_cell);
