  ...
  float blurring) 
{
  ...
  for(int row=0; row<beliefs.size(); row++){
  	for(int col=0; col<beliefs[0].size(); col++){
    	int new_i = (row + dy) % beliefs.size();
      	int new_j = (col + dx) % beliefs[0].size();
      	newGrid[new_i][new_j] = newGrid[row][col];
    }
  }
	...
	for(int row=0; row<beliefs.size(); row++){
    	for(int col=0; col<beliefs[0].size(); col++){
        	int hit;
          	...
          	newGrid[row][col] = beliefs[row][col] *(hit*p_hit + (1-hit)*p_miss);
        }
    }
  	float new_sum = 0.0;
  	for(int row=0; row<beliefs.size(); row++){
    	for(int col=0; col<beliefs[0].size(); col++){
        	new_sum += newGrid[row][col];  
        }
    }
  	for(int row=0; row<beliefs.size(); row++){
    	for(int col=0; col<beliefs[0].size(); col++){
        	newGrid[row][col] = newGrid[row][col]/new_sum;  
        }
    }
